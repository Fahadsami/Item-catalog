...
from flask import request, redirect, jsonify, make_response, flash
...
def createUser(login_session):
    ...
    name'''], email=login_session['email'], picture=login_session['picture'])
    session.add(newUser)
    session.commit()
...
def showCategory(catalog_id):
    ...
    (CategoryItem).filter_by(category_id=catalog_id).all()
...
(name=request.form['name'], description=request.form['''descri
ption'''], category_id=request.form['''cate
gory'''], user_id=login_session['user_id'])
...
@app.route
('/catalog/<int:catalog_id>/items/<int:item_id>/edit', methods=['GET', 'POST'])
...
Item''', catalog_id=categoryItem.category_id, item_id=categoryItem.id))
else:
    return render_template('''editCategory
    Item.html''', categories=categories, categoryItem=categoryItem)


@app.route('''/catalog/<int:catalog_id>/
items/<int:item_id>/delete''', methods=['GET', 'POST'])
...
return redirect(url_for('showCategory', catalog_id=categoryItem.category_id))
    ...
    CategoryItem.html''', categoryItem=categoryItem)
...
def fbconnect():
    ...
    if request.args.get('state') ! = login_session['state']:
        ...
        &client_id=%s&client_secret=%s&fb_
...
def showCategoryJSON(catalog_id):
    ...
    (CategoryItem).filter_by(category_id=catalog_id).all()
return jsonify
(categoryItems=[categoryItem.serialize for categoryItem in categoryItems])
